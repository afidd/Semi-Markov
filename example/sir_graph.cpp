...
{
  BuildGraph<SIRGSPN> single;
  BuildGraph<SIRGSPN> total;
  using Edge=BuildGraph<SIRGSPN>::PlaceEdge;

  enum { s, i, r };
  enum {infect_me, infect_you, recover};

  int64_t individual_idx=0;
  for (int64_t place : std::vector<int>{s, i, r}) {
    single.AddPlace({place, individual_idx}, 0);
  }

  single.AddTransition({infect_me},
   {Edge{{s, 0}, -1}, Edge{{i, 0}, +1}},
   std::shared_ptr<SIRTransition(nullptr));

  single.AddTransition({infect_you},
   {Edge{{i, 0}, -1}, Edge{{i, 0}, +1}},
   std::shared_ptr<SIRTransition(nullptr));

  single.AddTransition({recover},
   {Edge{{i, 0}, -1}, Edge{{r, 0}, +1}},
   std::shared_ptr<SIRTransition(new Recover()));

  std::map<int,BuildGraph<SIRGSPN>*> individual_graphs;
  individual_graphs[0]=&single;

  std::vector<int> individuals(individual_cnt);
  for (int i=0; i<individual_cnt; ++i) {
    individuals[i]=i;
  }

  using NodeToSubgraph=std::map<int,int>;
  NodeToSubgraph individual_type;
  int individual=0;
  for (int j=0; j<individual_cnt; ++j) {
    individual_type[j]=individual;
  }

  CompleteGraph<int> complete_maker;
  using ContactGraph=CompleteGraph<int>::GraphType;
  ContactGraph contact_graph=complete_maker.Create(individuals);

  using GraphNodeName=
      boost::property_map<ContactGraph,boost::vertex_index_t>::type;
  TransitionFusion<SIRGSPN,ContactGraph,GraphNodeName,NodeToSubgraph> fusion;

  TransitionFusion<SIRGSPN,ContactGraph>::Rules rules;
  rules.Add( individual, {infect_me}, individual, {infect_you},
    {infect}, std::shared_ptr<SIRTransition>(new Infect()) );

  GraphNodeName node_name=boost::get(boost::vertex_index_t(), contact_graph);
  BuildGraph<SIRGSPN> total=fusion.Fuse(individual_graphs, contact_graph,
      node_name, individual_type, rules);

  // std::move the transitions because they contain unique_ptr.
  return std::move(total.Build());
}
